...
class DolphinConnection extends events.EventEmitter {
  ...
  async initClient() {
    ...
    this.enet = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('enet')); });
    this.client = this.enet.createClient({
      peers: MAX_PEERS,
      channels: 3,
      down: 0,
      up: 0
  ...
  async connect(ip, port) {
    ...
    this.ipAddress = ip;
    this.port = port;
    this.peer = (_this$client = this.client) == null ? void 0 : _this$client.connect({
